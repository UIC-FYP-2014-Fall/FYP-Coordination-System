				...
				<div class="row-fluid">
					...
					<div class="block">
						...
						<div class="block-content collapse in">
							...
							<form
								action="${pageContext.request.contextPath }/setWorkload.do?flag=saveWorkload"
								method="post">
									...
									<tbody>
										<c:forEach items="${teacherList }" var="teacher"
											...
											<tr>
												...
												<td><select name="workload" class="input-small">
														<option value="${teacher.tid },1"
															<c:if test="${teacher.workload=='1' }">selected</c:if>>1</option>
														<option value="${teacher.tid },2"
															<c:if test="${teacher.workload=='2' }">selected</c:if>>2</option>
														<option value="${teacher.tid },3"
															<c:if test="${teacher.workload=='3' }">selected</c:if>>3</option>
														<option value="${teacher.tid },4"
															<c:if test="${teacher.workload=='4' }">selected</c:if>>4</option>
														<option value="${teacher.tid },5"
															<c:if test="${teacher.workload=='5' }">selected</c:if>>5</option>
												</select></td>
											</tr>
										</c:forEach>
									</tbody>
